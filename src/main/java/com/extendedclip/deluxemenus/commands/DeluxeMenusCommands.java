  ...
  public boolean onCommand(
    ...
    if (args[0].equalsIgnoreCase("help")) {
      ...
      if (sender instanceof ConsoleCommandSender) {
        ...
        final var menusList = menus.stream().map(menu -> menu.getMenuCommands() == null || menu.getMenuCommands().isEmpty()
            ? text(menu.getMenuName(), NamedTextColor.DARK_AQUA)
            .append(text(" - ", NamedTextColor.GRAY))
            .append(text("No menu command", NamedTextColor.RED))

            : text(menu.getMenuName(), NamedTextColor.DARK_AQUA)
            .append(text(" - ", NamedTextColor.GRAY))
            .append(text("/" + menu.getMenuCommands().get(0), NamedTextColor.GREEN))
      ...
      // Add all the menus using streams
      final var menusList = menus.stream().map(menu -> menu.getMenuCommands() == null || menu.getMenuCommands().isEmpty()
          ? text(menu.getMenuName(), NamedTextColor.DARK_AQUA).hoverEvent(
              HoverEvent.showText(text("No open command", NamedTextColor.GOLD)))
          : text(menu.getMenuName(), NamedTextColor.DARK_AQUA).hoverEvent(
              HoverEvent.showText(text("Open Command: ", NamedTextColor.GOLD)
                  .append(text("/" + menu.getMenuCommands().get(0), NamedTextColor.YELLOW))))
          .clickEvent(
              ClickEvent.suggestCommand("/" + menu.getMenuCommands().get(0)))
      ).collect(Component.toComponent(text(", ", NamedTextColor.WHITE)));
